...
breed [cytokines cytokine]         ; this is what we add to this model, we assume this cytokine is the TNF-alpha, which is a kind of pro-inflammatory cytokine
...
; we made a small change here, let the macrophages come to the wound area after 6 hours rather than 24 hours
    if any? patches with [debris > 0 and ticks > 6] [
...
; Here is what we add to the initial model, we assume that at the fourth hour (normal) or the second hour (stimulated) after the macrophage appeared, it will produce 1 unit (normal) or 2 units (stimulated) of pro-inflammatory cytokines
; with the restriction that there is still bacteria left in this area
    if Cytokine_production = "Normal" [
      if (any? macrophages with [age = 4]) and (any? bacteria) [
        create-cytokines 1 * count macrophages with [age = 4] [setxy random-xcor random-ycor setxy pycor pxcor set shape "star" set age 1 set color yellow]
      ]
    ]
    if Cytokine_production = "Stimulated" [
      if (any? macrophages with [age = 2]) and (any? bacteria) [
      create-cytokines 2 * count macrophages with [age = 2] [setxy random-xcor random-ycor setxy pycor pxcor set shape "star" set age 1 set color yellow]
      ]
    ]
; Here is how the pro-inflammatory cytokine works, it will activate the latent neutrophils and macrophages. If there is still any bacteria, it will activate 1 unit neutrophil and 1 unit macrophage
; If there is no bacteria left, it will activate 1 unit macrophage to remove debris
  if any? cytokines with [age = 1][
    ifelse any? bacteria
   [
      create-neutrophils 1 * count cytokines with [age = 1] [setxy random-xcor random-ycor setxy pycor pxcor set shape "neutrophil" set age 1]
      create-macrophages 1 * count cytokines with [age = 1] [setxy random-xcor random-ycor setxy pycor pxcor set shape "macrophage" set age 1]
   ]
   [
      create-macrophages 1 * count cytokines with [age = 1] [setxy random-xcor random-ycor setxy pycor pxcor set shape "macrophage" set age 1]
     ]
  ...
  ask cytokines with [age > 12][if 100 * (age / 24) > random 100 [die]] ; we assume the lifespan of the cytokines is 12 hours
...
to migrate
  ...
  [
    move-to one-of neighbors
  ] ;cytokines do not kill bacteria or remove debris, they can move to a random neighboring patch
end
...
to Leukocyte-Actions
  ...
  export-world "D:/BME2021Spring/BME 6311/problem sets/ModelProject/Stimulated.csv"
end
@#$#@#$#@
GRAPHICS-WINDOW
623
...
604
339
Change the production of cytokine
12
23.0
1
